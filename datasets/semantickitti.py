...
class SemanticKitti(Dataset) :
    ...
    def __init__(self, config, split='train', adapt=False) -> None: 
        """split=[train, val, test]"""
        super().__init__()
        
        self.config = config
        self.name = 'Semantic Kitti'
        ...
        self.dataset_path = os.path.join(self.root_dir, 'dataset', 'sequences')
        ...
        self.files = {
            'input' : flatten([[os.path.join(sequence, 'velodyne', frame) for frame in os.listdir(os.path.join(self.dataset_path, sequence, 'velodyne'))] for sequence in self.sequences]),
            }
    ...
    def __getitem__(self, index) :
        ...
        sparse_points, sparse_features, sparse_labels, sparse_indices, reverse_indices = ME.utils.sparse_quantize(coordinates=pos, features=features, labels=label, 
...
def semantickitti_collate(data) :
      ...
      return {
          ...
          'index' : batch_index,
          'reverse_indices' : batch_reverse_indices,
      }
...
if __name__ == "__main__" :
    ...
    config_file = r'/mnt/home/lamiae/CMMD/3DCMMD/configs/training/semantickitti.yaml'
    ...
    sk = SemanticKitti(
        ...
        config=config,
        root_dir=r'/mnt/home/lamiae/datasets/SemanticKITTI',
         voxel_size=0.1,
        augment=True,
        split='train'
    )
