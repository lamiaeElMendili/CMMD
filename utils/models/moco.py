...
def list_segments_points(p_coord, p_feats, labels, batch_size=1):
    ...
    for batch_num in range(batch_size):
        for segment_lbl in np.unique(labels):
            if segment_lbl == -1:
                continue
...
class MoCo(nn.Module):
    def __init__(self, model_head, K=65536, m=0.99999, T=1.0, config=None):
        ...
        self.register_buffer('queue_pcd', torch.randn(d,K))
        if self.normalization:
            self.queue_pcd = nn.functional.normalize(self.queue_pcd, dim=0)

        self.register_buffer('queue_seg', torch.randn(d,K))
        if self.normalization:
            self.queue_seg = nn.functional.normalize(self.queue_seg, dim=0)
    
        self.register_buffer('k_labels', torch.zeros(K, dtype=torch.long))
    ...
    def forward(self, source_stensor, source_labels, target_stensor, target_pseudo, step=None):
        ...
        if self.config.adaptation.cmmd.query == 'target student' :   
            _, h_q = self.model_q(target_stensor, is_seg=False)  # queries: NxC

            h_qs = list_segments_points(h_q.C, h_q.F, target_pseudo, self.batch_size)
            q_labels = target_pseudo[target_pseudo != -1]

        elif self.config.adaptation.cmmd.query == 'source student' :
            _, h_q = self.model_q(source_stensor, is_seg=False)
            h_qs = list_segments_points(h_q.C, h_q.F, source_labels, self.batch_size)
            q_labels = source_labels[source_labels != -1]

        elif self.config.adaptation.cmmd.query == 'source teacher' :
            _, h_q = self.model_k(source_stensor, is_seg=False)
            h_qs = list_segments_points(h_q.C, h_q.F, source_labels, self.batch_size)
            q_labels = source_labels[source_labels != -1]
        elif self.config.adaptation.cmmd.query == 'target teacher' :
            _, h_q = self.model_k(target_stensor, is_seg=False)
            h_qs = list_segments_points(h_q.C, h_q.F, target_pseudo, self.batch_size)
            q_labels = target_pseudo[target_pseudo != -1]
        ...
        if self.config.adaptation.cmmd.union_query and self.config.adaptation.cmmd.query == 'source student' :
            _, h_q = self.model_q(target_stensor, is_seg=False)
            h_qt = list_segments_points(h_q.C, h_q.F, target_pseudo, self.batch_size)

            z_qt = self.head_q(h_qt)
            if self.normalization :
                q_segt = nn.functional.normalize(z_qt.F, dim=1)
            else :
                q_segt = z_qt.F


            q_seg = torch.cat((q_seg, q_segt), dim=0)
            q_labels = torch.cat((q_labels, target_pseudo[target_pseudo != -1]), dim=0)
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            if self.config.adaptation.cmmd.key == 'source teacher queue' :
                ...
                _, h_k = self.model_k(source_stensor, is_seg=False)
                ...
                _, h_k = self.model_k(target_stensor, is_seg=False)
                ...
                _, h_k = self.model_q(source_stensor, is_seg=False)
                h_ks = list_segments_points(h_k.C, h_k.F, source_labels, self.batch_size)
                k_labels = source_labels[source_labels != -1]
            
            elif self.config.adaptation.cmmd.key == 'target student queue' :
                _, h_k = self.model_q(target_stensor, is_seg=False)
                h_ks = list_segments_points(h_k.C, h_k.F, target_pseudo, self.batch_size)
                k_labels = target_pseudo[target_pseudo != -1]
        ...
        return q_seg, q_labels, self.queue_seg.clone().detach(), self.k_labels.clone().detach()
